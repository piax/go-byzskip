...
func NewAdversaryRoutingTable(keyMV bs.KeyMV) bs.RoutingTable {
	return &AdversaryRoutingTable{normal: bs.NewSkipRoutingTable(keyMV), adversarial: bs.NewSkipRoutingTable(keyMV)} //, nodes: make(map[int]*BSNode)}
}

func (table *AdversaryRoutingTable) KeyMV() bs.KeyMV {
	return table.normal.KeyMV()
}
...
func (table *AdversaryRoutingTable) JSONString() string {
	ret := fmt.Sprintf("{\"key\":%s, \"mv\": \"%s\", \"faulty\": true, \"nls\":[", table.normal.KeyMV().Key(), table.normal.KeyMV().MV())
	nbrs := table.adversarial.GetNeighborLists()
	for i, sl := range nbrs {
		ret += sl.JSONString()
		if i != len(nbrs)-1 {
			ret += ",\n"
