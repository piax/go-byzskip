...
func (n *P2PNode) onReceiveMessage(s network.Stream) {
	...
	ayame.Log.Debugf("%s: Received ping request from %s. Message: %s", s.Conn().LocalPeer(), s.Conn().RemotePeer(), mes)
...
func (n *P2PNode) authenticateMessage(message *p2p.Message, data *p2p.MessageData, s network.Stream) bool {
	...
	senderBin, err := proto.Marshal(data)
	if err != nil {
		log.Println(err, "failed to marshal pb message")
		return false
	}
	...
	if err != nil {
		log.Println(err, "Failed to decode sender node id from base58")
		return false
	}

	authorId, err := peer.Decode(data.Author.Id)
	if err != nil {
		log.Println(err, "Failed to decode author node id from base58")
		return false
	}
	...
	n.verifyData(senderBin, []byte(message.SenderSign), senderId, senderPubKey)

	return n.verifyData(bin, []byte(authorSign), authorId, authorPubKey)
}
...
func (n *P2PNode) NewMessage(messageId string, mtype p2p.MessageType, key ayame.Key) *p2p.Message {
